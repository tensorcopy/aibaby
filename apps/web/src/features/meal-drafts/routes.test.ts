...
async function importMealDraftConfirmationRoute() {
  return import(`../../../app/api/meal-records/[mealRecordId]/confirm/route.ts?test=${Date.now()}-${Math.random()}`);
}
...
test("POST /api/meal-records/[mealRecordId]/confirm confirms or corrects a draft meal record", async () => {
  const calls: Array<unknown> = [];
  ...
  setMealDraftRouteDependenciesForTest({
    ...
    async confirmDraftMealRecord(input: Record<string, unknown>) {
      calls.push(input);
      return {
        mealRecord: {
          id: "meal_123",
          ...
          eaten_at: "2026-03-13T04:10:00.000Z",
          raw_text: "half a bowl of noodles and two pieces of beef",
          ai_summary: "Confirmed and corrected a dinner record with noodles, beef stew, and broccoli.",
          confidence_score: 1,
        ...
        generationIngestionEvent: {
          updated_at: "2026-03-13T04:13:00.000Z",
        },
      };
    },
  });

  const response = await (await importMealDraftConfirmationRoute()).POST(
    new Request("http://localhost/api/meal-records/meal_123/confirm", {
      method: "POST",
      headers: {
        "content-type": "application/json",
      },
    ...
    {
      params: Promise.resolve({ mealRecordId: "meal_123" }),
    },
  );
  ...
  const payload = await response.json();
  assert.equal(payload.mealRecord.status, "edited");
  assert.equal(payload.mealRecord.requiresConfirmation, false);
});
